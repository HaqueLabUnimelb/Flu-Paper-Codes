#!/usr/bin/env Rscript
# Purpose: run multi-replicate RCTD for the two day-4 spatial samples.
# Inputs: FLU_REFERENCE_RDS, FLU_DAY4_PUCK1_RDS and FLU_DAY4_PUCK2_RDS.
# Output: Day4mouseRCTDoutmin30_multi.rds in FLU_RCTD_OUTPUT_DIR.
...
puck_locs <- c(
  env_path(
    "FLU_DAY4_PUCK1_RDS",
    file.path(repo_dir, "data", "processed", "lungST_2_2_QCed_edit.rds")
  ),
  env_path(
    "FLU_DAY4_PUCK2_RDS",
    file.path(repo_dir, "data", "processed", "lungST_2_3_QCed_edit.rds")
  )
)
...
ref <- ref[, ref$timepoint_ref == "4dpi"]
...
output_file <- file.path(rctd_save_loc, "Day4mouseRCTDoutmin30_multi.rds")
